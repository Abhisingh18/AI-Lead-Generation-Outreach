// All countries with WhatsApp dial codes. `iso2` is used to render the flag emoji.
// `name` is used in the Google Maps query; `dial` is the WhatsApp country code.

export type Country = { name: string; iso2: string; dial: string };

// Most-used markets first (for convenience), then the rest alphabetically.
...
// Popular markets first, then everything else (skipping popular duplicates).
const popularNames = new Set(POPULAR.map((c) => c.name));
export const COUNTRIES: Country[] = [
  ...POPULAR,
  ...ALL.filter((c) => !popularNames.has(c.name)),
];
